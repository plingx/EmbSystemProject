...
uint32_t ETH_Init(ETH_InitTypeDef* ETH_InitStruct, uint16_t PHYAddress)
{
  ...
  {  
    ...
    {
      timeout++;
    } while (!(ETH_ReadPHYRegister(PHYAddress, PHY_BSR) ) && (timeout < PHY_READ_TO));
    ...
    /* Wait until the auto-negotiation will be completed *///& PHY_AutoNego_Complete)
    do
    {
      timeout++;
    }
		while (!(ETH_ReadPHYRegister(PHYAddress, PHY_BSR)) && (timeout < (uint32_t)PHY_READ_TO));  
