...
//bit4~0:保留
u8 tcp_server_flag;	 //TCP Server 测试全局状态标记变量
...
err_t tcp_server_poll(void *arg, struct tcp_pcb *tpcb)
{
	...
	if(es!=NULL)
	{
		if(tcp_server_flag&(1<<7))	//判断是否有数据要发送strlen((char*)tcp_server_sendbuf)
		{
			es->p=pbuf_alloc(PBUF_TRANSPORT,tcp_server_buffer.tcp_senCount,PBUF_POOL);//申请内存
		...
		tcp_server_buffer.tcp_senCount=0;	
		tcp_server_buffer.tcp_senWrite=0;
		memset(tcp_server_buffer.tcp_senbuf,0,TCP_SERVER_RX_BUFSIZE);//清空
		ret_err=ERR_OK;
	}else
	{
